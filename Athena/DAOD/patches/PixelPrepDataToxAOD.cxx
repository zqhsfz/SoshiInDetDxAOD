...
//    std::cout << "STSTST start looping rdo " << chList.size() << " " << totList.size() << std::endl;
//    for (int i=0; i<totList.size(); i++) {
//      std::cout << "STSTST totList " << totList[i] << std::endl;
//    }
//  }
...
/*
    ...
    float ANew = 0.0;
    float ENew = 0.0;
    float CNew = m_calibSvc->getQ2TotC(rId);
    if (chList[i]+CNew!=0.0) { 
      ANew = m_calibSvc->getQ2TotA(rId);
      ENew = m_calibSvc->getQ2TotE(rId);
      totCalc = ANew*(chList[i]+ENew)/(chList[i]+CNew);
    }
    std::cout << "STSTST inside " << CNew << " " << ANew << " " << ENew << " " << chList[i] << " " << totList[i] << " " << totCalc << std::endl;
    i++;
*/
